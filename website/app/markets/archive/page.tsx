...
export default async function MarketsArchivePage() {
  const supabase = getSupabase();
  const { data: editions, error } = await supabase
    .from("markets_editions")
      ...
      <div style={{ maxWidth: "620px", margin: "0 auto", backgroundColor: "#ffffff", padding: "48px" }}>
        ...
        <div style={{ marginBottom: "32px" }}>
          ...
          <p style={{ fontFamily: "Arial, sans-serif", fontSize: "13px", color: "#888888", marginLeft: "13px" }}>
            <Link href="/markets" style={{ color: "#888888", textDecoration: "none", borderBottom: "1px solid #cccccc" }}>
              ← Subscribe
            </Link>
          </p>
        </div>
        ...
        {editions && editions.map((edition: Edition) => (
          <div key={edition.id}>
            <Link href={`/markets/archive/${edition.slug}`} style={{ textDecoration: "none", display: "block" }}>
